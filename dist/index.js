...
**TIP**: If you don't know the exact path, use 'search_google_cloud_docs' first to find relevant documentation.`,
        inputSchema: {
            ...
            properties: {
                path: {
                    type: "string",
                    description: "The documentation path after cloud.google.com/ (e.g., 'compute/docs/instances/create-start-instance', 'storage/docs/creating-buckets')",
                },
            },
            required: ["path"],
        },
...
- Infrastructure: terraform, deployment manager, gcloud`,
        inputSchema: {
            ...
            properties: {
                query: {
                    type: "string",
                    description: "Free-form search query in natural language (e.g., 'how to share encrypted bucket cross account', 'vpc peering between projects', 'cloud sql high availability')",
                },
                product: {
                    type: "string",
                    description: "Optional: Filter by Google Cloud product (e.g., 'compute', 'storage', 'bigquery', 'kubernetes', 'sql', 'run')",
                },
            },
            required: ["query"],
        },
...
- Get BigQuery datasets API: service="bigquery", resource="datasets"`,
        inputSchema: {
            ...
            properties: {
                service: {
                    type: "string",
                    description: "The Google Cloud service name (e.g., 'compute', 'storage', 'bigquery', 'pubsub', 'sql', 'kubernetes', 'functions', 'run', 'iam')",
                },
                resource: {
                    type: "string",
                    description: "Optional: Specific API resource (e.g., 'instances', 'buckets', 'datasets', 'topics')",
                },
            },
            required: ["service"],
        },
...
async function fetchGoogleCloudDoc(path) {
    ...
    try {
        ...
        if (!response.ok) {
            return JSON.stringify({
                ...
                url: url,
                suggestion: "Please check the path and try again. Use 'list_google_cloud_products' to see available products.",
            });
        }
        ...
        $("script, style, nav, header, footer, noscript, iframe, svg, img, " +
            ...
            ".buttons, .button-group, .cta, .feedback, .rating").remove();
        // Extract title from h1 or title tag
        const title = $("h1.devsite-page-title").first().text().trim() ||
            $("article h1").first().text().trim() ||
            $("h1").first().text().trim() ||
            $("title").text().replace(" | Google Cloud", "").trim() ||
            "Google Cloud Documentation";
        // Find main content area
        const mainContent = $(".devsite-article-body").html() ||
        ...
        $content("nav, .devsite-nav, .devsite-toc, .nocontent, " +
        ...
        $content("h1, h2, h3, h4, h5, p, pre, ul, ol, table, blockquote, dl").each((_, elem) => {
            ...
            if (text.includes("Documentation") && text.includes("Home") && text.length < 100)
                return;
            if (text.startsWith("Stay organized with collections"))
                return;
            if (text.startsWith("Save and categorize"))
                return;
...
async function searchGoogleForCloudDocs(query, product) {
    ...
    try {
        ...
        $("div.g, div[data-hveid]").each((_, elem) => {
            ...
            // Extract snippet
            const snippet = $elem.find("div[data-sncf], div.VwiC3b, span.aCOpRe").first().text().trim() ||
        ...
        // Deduplicate by URL
        const uniqueResults = results.filter((item, index, self) => index === self.findIndex((t) => t.url === item.url));
...
async function searchCloudGoogleDirect(query) {
...
function calculateRelevance(url, title, query) {
    ...
    if (urlLower.includes("/how-to/") || urlLower.includes("/configure"))
        score += 2;
    if (urlLower.includes("/quickstart"))
        score += 1;
    // Penalty for very generic pages
    if (urlLower.endsWith("/docs") || urlLower.endsWith("/docs/"))
        score -= 1;
    return score;
}
async function searchGoogleCloudDocs(query, product) {
    const results = [];
...
function listGoogleCloudProducts() {
    ...
    return JSON.stringify({
        ...
        products: products,
        usage: 'Use "fetch_google_cloud_doc" with the docsPath to get documentation content.',
    });
}
async function getApiReference(service, resource) {
    ...
    if (!product) {
        return JSON.stringify({
            ...
            suggestion: 'Use "list_google_cloud_products" to see all available services.',
        });
    }
